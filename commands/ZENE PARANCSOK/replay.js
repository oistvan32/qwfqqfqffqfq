...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!queue) return embedbuilder("null", message, config.colors.no, "Nincs hallható zene!");

    //get current song
    let cursong = queue.songs[0];

    //send information message
    functions.embedbuilder(client, 5000, message, config.colors.yes, "Az aktuális zene újra játszása:", `[${cursong.name}](${cursong.url})`, cursong.thumbnail)
