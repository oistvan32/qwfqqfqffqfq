...
module.exports = (client) => {
    ...
    client.on("ready", () => {
        setInterval(() => {
            ...
            if (stateswitch) client.user.setActivity(`${config.prefix}help | ${client.guilds.cache.size} Szerver`, {
                type: "PLAYING"
            });
            else client.user.setActivity(`${config.prefix}help | ${client.guilds.cache.size} Szerver`, {
                type: "PLAYING"
            });
    ...
    function getAll(client, guild) {
        const embed = new MessageEmbed()
            ...
            .setTitle('**__Help parancsok - Köszönöm,hogy be hívtál!__**')
            .addField("**__BOT BY:__**", `
                >>> [\`Support\`](https://discord.gg/JJ5gUUfj2e) [\`INVITE\`](https://discord.com/oauth2/authorize?client_id=870348656883167324&permissions=8&scope=bot)
                `)
            .addField("**__Zene - Támogatott appok__**", `
                >>> \`Youtube\`, \`Soundcloud\`, \`Spotify\`, ...*HAMAROSAN*
                `)
            .setFooter("Több infóért:+help [PARANCS NÉV]", client.user.displayAvatarURL())
        ...
        return channel.send(embed.setDescription(`*Használd a prefixet: **\`${config.prefix}\`** Minden parancs elejére használd a prefixet,hogy a parancs végre hajtodjon*\n` + info));
    }
    client.on('voiceStateUpdate', (oldState, newState) => {
        if (newState.id === client.user.id && oldState.serverDeaf === true && newState.serverDeaf === false) {
            try {
                ...
                channel.send("Ne unmutolj engem! Úgy is gyorsabb vagyok és vissza rakom magam Lul!")
